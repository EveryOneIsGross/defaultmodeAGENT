...
async def call_gemini_api(content, context, system_prompt, temperature, is_image):
    ...
    try:
            ...
            if isinstance(content, list):
                for item in content:
                    if isinstance(item, dict) and item.get("type") == "image":
                        ...
                        parts.append(types.Image.from_bytes(img_bytes, mime_type="image/jpeg"))
                    elif isinstance(item, dict) and item.get("type") == "text":
                        parts.append(item["text"])
            else:
                raise ValueError("Invalid image content format for Gemini API")
        else:
            # Handle text content
        ...
        response = api.gemini_client.models.generate_content(
            model=api.model_name,
            contents=parts,
            config=config
        )
