...
NEGATIONS = {
    ...
    "doesn't", "didn't", "shouldn't", "wouldn't", "couldn't", "mustn't",
    "haven't", "hasn't", "hadn't", 'unless', 'except'
}
...
def is_word(t): return bool(WORD_RE.fullmatch(t))
def is_anchor(t): return bool(ANCHOR_RE.match(t))
...
def build_edge_graph(content_words, horizon=6, decay=0.7):
    ...
    n = len(content_words)
    for i in range(n):
        for j in range(i + 1, min(i + horizon + 1, n)):  # FIXED off-by-one
            dist = j - i
            weight = decay ** (dist - 1)
...
def build_node_edge_sums(edge_scores):
    s = defaultdict(float)
    for (w1, w2), sc in edge_scores.items():
        s[w1] += sc
        s[w2] += sc
    return s

def is_noun_after_adjective(words, idx):
    return idx > 0 and words[idx - 1].lower() in ADJECTIVES

def compute_word_weight(word, idx, words, base_salience, compression_for_scoring):
    wl = word.lower()
    pf = pidgin_factor(compression_for_scoring)
    if wl in NEGATIONS:
        return base_salience * (2.0 - pf * 0.3)
    if wl in ARTICLES:
        return 0.01 if pf > 0.3 else 0.1
    if wl in FUNCTION_WORDS:
        return 0.1 * (1.0 - pf * 0.5)
    w = base_salience
...
def extract_spanning_path(words, keep_ratio, node_salience, node_edge_sum, protected_bigrams,
                          compression_for_scoring, compression_for_pidgin):
    if len(words) <= 2:
        return set(range(len(words)))
    pf_pidgin = pidgin_factor(compression_for_pidgin)
    word_weights = {}
    for i, w in enumerate(words):
        wl = w.lower()
        base = node_salience.get(wl, 1.0)
        base_salience = base + node_edge_sum.get(wl, 0.0)
        word_weights[i] = compute_word_weight(w, i, words, base_salience, compression_for_scoring)
...
def compress_sentence(sentence, node_salience, node_edge_sum, keep_ratio, protected_bigrams,
                      compression_for_scoring, compression_for_pidgin):
    ...
    if len(words) <= 2:
        if pf_pidgin > 0.5 and len(words) == 2:
            content = [w for w in words if (w.lower() not in FUNCTION_WORDS) or (w.lower() in NEGATIONS)]
            if content:
                return ' '.join(content)
        return sentence.strip()

    kept_positions = extract_spanning_path(
        words, keep_ratio, node_salience, node_edge_sum, protected_bigrams,
        compression_for_scoring, compression_for_pidgin
    )
    ...
    content_words = [
        w for w in tokenize_text(result)
        if is_word(w) and (w.lower() not in FUNCTION_WORDS or w.lower() in NEGATIONS)
    ]

    # RESTORED: global pidgin can delete a whole thin sentence (this is the old last-10% knife)
    if len(content_words) < 2 and pf_pidgin > 0.5:
        return ""

    return result

def terminal_pidgin_pass(text, compression):
    pf = pidgin_factor(compression)
    if pf < 0.6:
        return text
    ...
    def flush_on_boundary():
        nonlocal run_content, run_has_anchor, out
        if pf > 0.7 and run_content < 2:
    ...
    for t in toks:
        ...
        if is_anchor(t) or wl in NEGATIONS:
            out.append(t)
            run_has_anchor = True
            run_content += 1
            continue
...
def fuzzy_post_compress(text, fuzzy_strength, compression):
    ...
    for idx, w in enumerate(words):
        ...
        for prev_word, prev_idx in candidates:
            ...
            bucket_dict.setdefault(prefix, []).append((wl, idx))
    ...
    for local_idx in drop_list:
        filtered[word_indices[local_idx]] = ""
...
def window_dedupe(text, window=WINDOW):
    ...
    for i, t in enumerate(tokens):
        if is_word(t):
            k = t.lower()
            if k in last_pos and i - last_pos[k] < window and k not in FUNCTION_WORDS and k not in NEGATIONS:
                out.append("")
            else:
                last_pos[k] = i
                out.append(t)
...
def chronomic_filter(text, compression=0.5, fuzzy_strength=1.0, horizon=6):
    ...
    node_edge_sum = build_node_edge_sums(edge_scores)
    ...
    for s, d in zip(sentences, densities):
        ...
        c = compress_sentence(
            s, node_salience, node_edge_sum, keep_ratio, protected_bigrams,
            compression_for_scoring=local_compression,
            compression_for_pidgin=compression
        )
        if c:
            compressed.append(c)

    out = " ".join(compressed)
    out = terminal_pidgin_pass(out, compression)
...
if __name__ == "__main__":
    ...
    if a.input:
        ...
        for c in [0.5, 0.7, 0.85, 0.95]:
            for fz in [0.0, 1.0, 2.0]:
                result = chronomic_filter(demo, compression=c, fuzzy_strength=fz)
                st = stats(demo, result)
                pf = pidgin_factor(c)
                print(f"\n=== c={c} fuzzy={fz} pidgin={pf:.2f} ({st['actual_compression']}) ===")
                print(result)
