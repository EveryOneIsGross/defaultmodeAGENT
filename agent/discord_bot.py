...
from hypocampus import Hypocampus, HypocampusConfig
...
def log_to_jsonl(data, bot_id=None):
    """Log data to JSONL file with consistent timestamp format.
    
    Args:
        data (dict): Data to log
        bot_id (str, optional): Bot identifier for log filename
    """
    # Ensure data has properly formatted timestamp
...
def update_temperature(intensity: int) -> None:
    ...
    logging.info(f"Updated bot temperature to {TEMPERATURE} across all components")
...
async def maintain_typing_state(channel):
    ...
    try:
        ...
        logging.debug(f"Typing state maintenance ended: {str(e)}")
...
async def process_message(message, memory_index, prompt_formats, system_prompts, github_repo, is_command=False):
    ...
    logging.info(f"Received message from {user_name} (ID: {user_id}): {sanitized_content}")
    ...
    try:
        ...
        try:
            ...
            # Initialize Hypocampus for reranking if we have candidates
            if candidate_memories:
                hypocampus_config = HypocampusConfig()
                hypocampus = Hypocampus(hypocampus_config)
                # Rerank memories with a threshold
                relevant_memories = await hypocampus.rerank_memories(
            ...
            # Build context directly from API and memories
            ...
            async for msg in message.channel.history(limit=MAX_CONVERSATION_HISTORY):
                ...
                messages.append(f" @{clean_name}: {truncated_content}")
        ...
        if response_content:
            ...
            await message.channel.send(formatted_content) if len(formatted_content) <= 1800 else await send_long_message(message.channel, formatted_content)
            logging.info(f"Sent response to {user_name} (ID: {user_id}): {response_content[:1000]}...")

        if response_content:
            timestamp = currentmoment()
            memory_text = f"({timestamp}) User @{user_name} in #{message.channel.name if hasattr(message.channel, 'name') else 'DM'}: {sanitize_mentions(sanitized_content, message.mentions)}"
...
async def process_files(message, memory_index, prompt_formats, system_prompts, user_message="", bot=None, temperature=TEMPERATURE):
    ...
    try:
        ...
        logging.info(f"Using amygdala arousal: {amygdala_response}")
        ...
        try:
            ...
            if response_content:
                ...
                logging.info(f"Sent file analysis response to {user_name} (ID: {user_id})")
            ...
            try:
                ...
                if e.status == 429:  # Rate limit hit
                    ...
                    if retry_count == max_retries:
                        logging.error("Max retries reached for message chunk. Skipping.")
                        break
                        
                    retry_after = getattr(e, 'retry_after', base_delay * (2 ** retry_count))
                    logging.warning(f"Rate limited. Waiting {retry_after:.2f}s before retry {retry_count}/{max_retries}")
...
async def generate_and_save_thought(memory_index, user_id, user_name, memory_text, prompt_formats, system_prompts, bot):
    ...
    logging.debug(f"Pre-memory addition string: {memory_string}")
    memory_index.add_memory(user_id, memory_string)
    logging.debug(f"Post-memory addition: {memory_index.user_memories[user_id][-1]}")
...
def setup_bot(prompt_path=None, bot_id=None):
    ...
    try:
        ...
        if bot_id:
            ...
            logging.info(f"Attempting GitHub init with token env: GITHUB_TOKEN_{bot_id.upper()}")
        else:
            github_token = os.getenv('GITHUB_TOKEN')
            github_repo_name = os.getenv('GITHUB_REPO')
            logging.info("Attempting GitHub init with default token env")

        if not github_token or not github_repo_name:
            bot.github_enabled = False
            github_repo = None
            logging.warning(f"GitHub credentials not found for bot {bot_id or 'default'}. Required env vars: " + 
            ...
            logging.info(f"GitHub integration enabled for repository: {github_repo_name}")
    except Exception as e:
        bot.github_enabled = False
        github_repo = None
        logging.warning(f"GitHub initialization failed for bot {bot_id or 'default'}: {str(e)}. GitHub features will be disabled.")
    ...
    async def on_ready():
        ...
        logging.info('DMN processor started')
        logging.info('------')
        log_to_jsonl({
            'event': 'bot_ready',
            'timestamp': datetime.now().isoformat(),
    ...
    async def set_amygdala_response(ctx, intensity: int = None):
        ...
        if intensity is None:
            ...
            logging.info(f"Amygdala arousal queried: {bot.amygdala_response}%")
            ...
            logging.info(f"Amygdala arousal set to {intensity}% across all components")
        else:
            await ctx.send("Please provide a valid intensity between 0 and 100.")
            logging.warning(f"Invalid amygdala arousal attempted: {intensity}")
    ...
    @commands.check(lambda ctx: (isinstance(ctx.channel, discord.DMChannel) or ctx.author.guild_permissions.manage_guild) and bot.github_enabled)
    ...
    async def kill_tasks(ctx):
        ...
        if has_permission:
            try:
                ...
                logging.info(f"Kill command initiated by {ctx.author.name} (ID: {ctx.author.id})")
            except Exception as e:
                await ctx.send(f"Error in kill command: {str(e)}")
                logging.error(f"Kill command error: {str(e)}")
        else:
    ...
    async def resume_tasks(ctx):
        ...
        if has_permission:
            ...
            logging.info(f"Processing resumed by {ctx.author.name} (ID: {ctx.author.id})")
        else:
    ...
    async def get_logs(ctx):
        """Download bot logs (Server management or DM only)."""
        try:
            safe_bot_name = sanitize_filename(bot.user.name)
            log_filename = f'bot_log_{safe_bot_name}.jsonl'
            temp_filename = f'temp_log_{safe_bot_name}.jsonl'
            
            # Look in current directory where logs are actually written
            log_path = log_filename  # <-- Should match where log_to_jsonl writes
                ...
                if recent_lines:
                    ...
                    try:
                        await ctx.author.send(
                            ...
                            file=discord.File(temp_path, filename=f"{safe_bot_name}_recent_logs.jsonl")
                        )
...
if __name__ == "__main__":
    ...
    logging.info(f"Using prompt path: {prompt_path}")
